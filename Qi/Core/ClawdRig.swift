...
enum ClawdRig {
    ...
    /// **漏改这一个，手就会长在肚子上。**
    /// ⚠️⚠️ **10 → 8**：肩膀从半腰抬到身体上 1/4（见 `scripts/抬肩膀.py`）。
    ///
    /// 她带回来那份对照表的第 ② 条：「肩膀位置｜正确：身体上 1/4｜
    /// 他的错误：在半腰，手臂像从肚子长出来」。
    /// 原来手臂中线在第 12 行 = 身子往下 35%，确实是半腰；
    ...
    private static func plainPlan(pose: CarryPose, beat: Double,
        ...
        case .lift:
            ...
            // ⚠️⚠️ **举着 = 稳稳托住不动，不是在做深蹲。**
            //
            // 她问：「你举床上下动你要怎么移动呢？
            // 别忘记我还需要 clawd 举起来之后自己找地方放下哈。」
            //
            // 一点没错，而且这是**逻辑**问题不是观感问题：
            // 举东西这一档是给「搬家具」用的——他要举着走到位置再放下
            // （走路和放下 `ClawdHomeView` 里本来就有）。
            // 一边无限做深蹲一边走路，根本讲不通。
            //
            // 所以这儿 `squat = 0`：举着的时候整只钉住，
            // 该动的交给走路那几帧。费劲靠**汗**来表达（见 `sweat`）——
            // 汗是唯一一处能看出「重」的东西，动作只能看出「在动」。
            //
            // 「使劲蹲下去再蹬起来」那一下属于**举起来的瞬间**，
            // 不属于举着的这一路，要做也是另开一档。
            p.armsUp = true
            p.squat = 0
    ...
    static let wearOrder: [WearSlot] = [.back, .body, .feet, .neck, .face, .head]
