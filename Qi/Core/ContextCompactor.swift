...
enum ContextCompactor {
    ...
    static func compactNow(_ conversationID: UUID, app: AppState) async -> Bool {
        ...
        for m in body {
            ...
            var text = m.content.count > 600
                ? String(m.content.prefix(600)) + "…" : m.content
            // 图也要进浓缩件的视野。
            //
            // 压缩之后这段原文就不再发了，图早就淡出成一行字，
            // **那一行字要是不进这份摘要，这张图就彻底没人提了**——
            // 她再问「还记得那张照片吗」，索引里连一个能搜的词都没有。
            // （原图本身没丢，`show_image` 照样找得到，前提是有词可搜。）
            if !m.imageNote.isEmpty {
                text += "（她发的图：\(m.imageNote)）"
            } else if !m.imageNames.isEmpty {
                text += "（她发了 \(m.imageNames.count) 张图）"
            }
            transcript += "\(who)：\(text)\n"
        }

        // ⑥ 那一块的写法单独拎出来说。**这是整块索引成不成立的关键**：
        // 索引里的词要是「日本行」这种概括词，而原文里她写的是
        // 「银座」「RIMOWA」「辣子炸河豚」，那 search_window 一个字都搜不到——
        // 索引就成了一份看着很齐全、用起来永远落空的目录。
        let indexRule = """
        ⑥ 的写法：
        · 每行是「话题：词、词、词」。
        · **词必须是原文里真的出现过的那几个字**——人名、地名、店名、菜名、
          型号、日期、她当时的原话里那个特别的说法。搜的就是这些字。
        · **不要写概括词**。「那次旅行」搜不到任何东西；
          「银座」「RIMOWA」「辣子炸河豚」才搜得到。
        · 只写这份浓缩件里**没写进去**的那些事——写进去的不用再搜一遍。
        · 十来行就够，挑她以后真可能回头问的。
        """
        ...
        ⑥ 【怎么翻回去】 —— 前面那些原文**没有删，只是不再发给我**，
        我随时可以用 `search_window` 拿关键词把它们原样翻回来。
        所以这一块写的是**钥匙**：一行一个话题，后面跟上能搜到它的词。
