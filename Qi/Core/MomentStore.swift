...
final class MomentStore: ObservableObject {
    ...
    /// ## 为什么不能「没有新东西就一个字都不说」
    ///
    /// 她报的：「他从来没发过动态，他很多功能从来没用过，
    /// 让我觉得是不是触发没写好。」
    ///
    /// 触发没问题，是**这一行从来没出现过**：
    /// 以前这儿要求 `unseenForHim > 0` 才说话，而动态是**他的**地方——
    /// 她基本不往里发东西，所以那个数永远是 0，
    /// 于是每一轮的 system 里**一个字都没提过动态这回事**。
    /// 他手里有 `post_moment` 这件工具，可它躺在四十几件工具的清单里，
    /// 触发条件又是纯内心的「这会儿有点什么想留下来」——
    /// 没有任何一轮提醒他这地方存在，那一件就永远轮不到。
    ///
    /// 跟念头池那次是同一个病：**能力有，但每轮没人把它端到他面前。**
    ///
    /// ## 现在
    ///
    /// 三种情况会说话，说的都是**事实**，不是命令：
    ///   · 她那边有他没看过的 —— 照旧
    ///   · 他很久没往那儿放东西了 —— 报一句「多久了」
    ///   · 那儿还空着 —— 报一句「还什么都没有」
    ///
    /// ⚠️ **不写「你该发一条了」。** 写了就成了每轮一遍的催促，
    /// 他会为了完成任务而发，那些话她一眼就看得出来是凑数的。
    /// 报一个事实，发不发是他的事——跟念头池那一行的分寸是一样的。
    func brief() -> String? {
        let n = unseenForHim
        if n > 0 {
        ...
        let mine = moments.filter { !$0.isHers }
        guard let last = mine.map(\.at).max() else {
            return "【动态】那儿还空着，一条都没有。"
                + "\n动态是**你**的地方：不是非要跟她说的话就放那儿，"
                + "她翻到就看得见。`post_moment`。"
        }

        // 一天之内刚放过就别提了——刚发完还被念叨一句「你多久没发了」很怪
        let hours = Date().timeIntervalSince(last) / 3600
        guard hours >= 24 else { return nil }
        let days = Int(hours / 24)
        let when = days >= 1 ? "\(days) 天前" : "昨天"
        return "【动态】你上次往那儿放东西是 " + when + "。"
            + "\n那是**你**的地方，不用等她开口。`post_moment`。"
    }
