...
struct IsoRoom {
    ...
    static func fit(in size: CGSize, as projection: RoomProjection,
        ...
        // 而她要的正是地板占得多。
        // ⚠️ 这个数改过三轮，每一轮都是跟着**地板的深浅**走的：
        //   9 → 她说「小屋有点太高了」，墙占了六成，家具缩在井底
        //   7 → 那时候地板还是 8 格深
    ...
    func walkBand(in size: CGSize) -> (top: Double, bottom: Double) {
        ...
        // ⚠️⚠️ **这儿只管地板，不替他留身子。**
        //
        // 「他半个身子在屋外」那件事我在这儿拍脑袋留过两次白
        //（先 0.03，再 `tileW * 0.55`），两次都不够——因为他到底多高
        // **这儿根本不知道**，那是画出来才有的数。
        //
        // 现在改成：这儿给的是**地板本身**的上下沿，
        // 留白由 `ClawdHomeView` 那边**量出他真实的高度**再扣
        //（见那边的 `bodyH` / `band`）。猜出来的数迟早不够。
...
struct IsoShape {
    ...
    /// 台面在这件东西**自己那么高**的百分之几处。
    ///
    /// 放在它上面的杯子该抬多高，算的就是这个（见 `IsoRoomView.mountLift`）。
    ///
    /// ⚠️ **不能拿 `tall` 直接当高度用。** `tall` 是「几格高」，
    /// 而一件东西画出来多高是按**图自己的长宽比**来的——
    /// 一张 `tall: 0.9` 的桌子画出来能有两格半高。
    /// 拿 `tall` 当高度算，杯子只会抬到桌腿的位置。
    ///
    /// ⚠️ 桌子、凳子的台面在半腰（底下是腿），
    /// 柜子、冰箱、洗衣机的顶就在最上面。一格以上的按顶算。
    var surfaceAt: Double { tall >= 1.0 ? 0.9 : 0.55 }
