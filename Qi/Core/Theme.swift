...
struct GlassSurface: View {
    ...
    // 因为背景暗下来毛玻璃就自然暗下来了。」
    //
    // 玻璃就是「把背后的画面糊一糊」，背后暗了它自然就暗；
    // 另外盖一层黑，等于在白玻璃上盖黑纱——出来是死灰，
    ...
    /// 模糊 ＝ **EVE 那份的配方**。她发来那份 docx 时说：只参考她的毛玻璃效果。
    ///
    ///     气泡        白 20% + blur(10px) + 1px 白边 30% + 外阴影 0 8px 32px rgba(0,0,0,.1)
    ///     顶栏/输入区  白 10% + blur(5px)，无边
    ///     输入框/按钮  白 30% + blur(5px) + 1px 白边 20%
    ///
    /// ⚠️⚠️ **她这套跟我们之前的方向是反的，这才是关键。**
    ///
    /// 我们一直在往「糊得更狠」上走（`regular`、`thick`），
    /// 而她的糊只有 5–10px——相当于系统**最薄**那一档。
    /// 毛玻璃的质感在她这儿是**那层白纱和那圈白边**撑起来的，不是糊出来的。
    /// 所以底换回 `.ultraThinMaterial`，白纱按她的数给。
    ///
    /// ⚠️ 外阴影（`0 8px 32px rgba(0,0,0,.1)`）**没做**。
    /// 阴影要画在玻璃**背后**，而玻璃会把背后的东西一起采样进来——
    /// 深色下那一下正好把她刚嫌过的「太黑」又加回去。
    /// 边和上缘高光已经能把形状读出来，缺这一层不影响这套配方成立。
    ///
    /// 深浅两套：深色下白纱减半（同样的 20% 白盖在暗底上是一块灰板）。
    private var recipeBlur: some View {
        let dark = scheme == .dark
        // 滑到底就是这套配方本身；往左拉才掺进没糊的原画面（见下面 `strength`）
        let k = min(1, max(0.3, strength))
        // ⚠️⚠️ **气泡那层白由「气泡不透明度」那根滑块说了算。**
        //
        // 她连着三次说「气泡还是透的」——不是配方没生效，
        // 是**能调到的上限太低**：EVE 那份给 20%，我提到 30%，
        // 而她那张壁纸本身就很淡，30% 的白盖上去跟没盖差不多。
        //
        // 所以这根滑块现在直接调这层白：拉满 55%（深色 30%），
        // 拉到底几乎不给。「不透明度」这三个字她要的本来就是这个，
        // 以前它只能把玻璃调得更透，永远调不厚。
        let veil = light
            ? (dark ? 0.03 + 0.15 * k : 0.05 + 0.25 * k)
            : (dark ? 0.09 * k : 0.15 * k)
        let edgeWhite = light ? (dark ? 0.18 : 0.30) : (dark ? 0.14 : 0.20)
        // ⚠️⚠️⚠️ **材质永远满格，不允许再拿滑块去稀释它。**
        //
        // 这儿曾经写的是 `.opacity(0.4 + 0.6 * k)`，意思是「滑块往下拖就更透」。
        // 她报的：「我要的是让气泡跟输入法一样变模糊，不是变白……
        // 就算我拖动不透明度的滑块，只是变白，稍微低一点还是直接透出背景。」
        //
        // 就是这一行干的：**材质调淡 = 把没糊过的原画面掺回来**，
        // 滑块拖到 0.5 就有三成背景是清晰的。而输入框不走这根滑块，
        // 所以它一直是满的、糊得好好的——她看到的差别就来自这儿。
        //
        // 现在：糊多少是定死的（跟输入框一样），滑块只管上面那层白。
        return shape.fill(.ultraThinMaterial)
            .overlay {
                // ⚠️ 她给的是**一层平的白**，不是上下渐变。
                // 渐变是我们自己加的花样，照抄就照抄到底。
                // ⚠️ 这儿不再乘 `k`：上面算 `veil` 的时候已经把它算进去了，
                // 再乘一遍等于平方，滑块拉到一半时白会掉到四分之一。
                shape.fill(.white.opacity(veil))
            }
            .overlay {
                if extra > 0.01 { shape.fill(.white.opacity(extra * 0.10)) }
            }
            .overlay {
                // border: 1px solid rgba(255,255,255,.3)。**气泡也要**——
                // 她那份里气泡是有边的，之前我们特地不给气泡描边，
                // 那是另一套（无边）配方留下的规矩，这一套不适用。
                shape.strokeBorder(.white.opacity(edgeWhite), lineWidth: 1)
            }
            .overlay {
                // 上缘那一线内高光留着：她那份没写，但它只在顶上一线，
                // 跟白边不打架，玻璃的「薄」靠它读出来
