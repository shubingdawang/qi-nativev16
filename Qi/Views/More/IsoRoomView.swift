...
struct IsoRoomView<Clawd: View>: View {
    ...
    private func floor(_ geoRoom: IsoRoom) -> some View {
        ...
        return ZStack(alignment: .topLeading) {
            ...
            // ⚠️ 平面屋再滴一圈实边。
            //
            // 她说的：「再加上边框，现在这样不太像里面有空间
            // 可以放家具的样子，像一张纸。」
            //
            // 上面那一条是给地板勾边的，淡得几乎看不见；
            // 这一条粗一点、深一点，把地板圈成一个「里面」。
            if geoRoom.projection == .flat {
                geoRoom.floorPath
                    .stroke(Color.black.opacity(scheme == .dark ? 0.42 : 0.22),
                            lineWidth: 2)
            }
        }
    }
    ...
    private func piece(_ item: Furniture, _ kind: FurnitureKind,
        ...
        // 一张图画多宽：**按它占几格算**，不看图本身多少像素。
        // 她导进来的图不管多大，摆在屋里都是这件家具该有的大小。
        //
        // ⚠️⚠️ **末尾那个 1.5 撤了。**
        //
        // 她报的：「地板的判定方格太大了，家具根本没法契合地板，
        // 像书架就是放在哪块上都没有靠墙，甚至直接占着两格到三格。」
        //
        // 一件 `w:1, d:1` 的书柜，乘完 1.5 就画成一格半宽——
        // 它当然贴不上墙，也压不进一格里。
        //
        // 当初加 1.5 是因为她说「有点小」，可那道题解错了地方：
        // 一个一人多高的柜子该显得高，靠的是**图自己的长宽比**
        //（`aspectRatio(.fit)` 会照着给高度），不是把它横着拉宽。
        // 拉宽只会让它跨格。
        //
        // ⚠️ 要是这么改完整体偏小，该动的是**格子本身**（`tileW`），
        // 不是这个系数——动格子，所有东西一起变大且依然对齐；
        // 动这儿，只有家具变大，而它跟地板的关系就散了。
        //
        // ⚠️ 两种投影下「一格」的宽度不一样：
        // 等距屋里一件 w×d 的东西底面横跨 `(w + d) / 2` 格，
        // 平面屋里它就是 `w` 格。用错的话平面屋里所有东西都偏窄。
        let footprint = geoRoom.projection == .flat
            ? CGFloat(max(1, s.w))
            : CGFloat(s.w + s.d) / 2
        let mineW = geoRoom.tileW * footprint
