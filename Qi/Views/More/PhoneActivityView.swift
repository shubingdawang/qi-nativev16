            ...
            if app.settings.checkInEnabled {
                ...
                HelpNote(title: "读取范围") {
                    ...
                    **仅在他调用该工具时读取一次**，不持续采集。
                    ...
                    读取内容：
                    · **屏幕** —— 快捷指令最近一次保存的截图（与「让他看屏幕」共用文件夹）。未配置则不提供。
                    · **位置** —— 按上方选项提供。「只到城市」仅含城市与区（如「日本 东京都 涩谷区」），不含街道；「不给位置」则不提供。
                    · **天气** —— 以经纬度向 Open-Meteo 查询。**这是唯一离开本机的数据**，仅发送经纬度，不含身份信息；该接口无需注册，不留查询记录。
                    · **手机使用时长** —— 读取本机数据。

                    ⚠️ 截图**非实时**。iOS 不允许 App 主动截取其他 App 的画面，读取到的是最近一次运行快捷指令时的画面，返回结果中会注明时间间隔。
                    """))
                        .font(.app(11))
                        .foregroundStyle(Theme.textMuted(scheme))
    ...
    private var screenPeekCard: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            HelpNote(title: "配置步骤") {
                ...
                **第一步 · 截图保存位置**
                默认为栖的文件夹：「文件」App → **我的 iPhone › 栖 › 给他看的屏幕**，无需另选。
                如需保存到其他位置，可点下方「改读别的文件夹」。

                **第二步 · 在「快捷指令」App 中新建快捷指令**
                1. 打开快捷指令，点右上角 ＋ 新建
                2. 添加操作「拍摄屏幕快照」
                3. 添加操作「存储文件」，目标选择 **我的 iPhone › 栖 › 给他看的屏幕**（或所选文件夹）
                4. 展开「存储文件」，关闭「询问存储位置」
                5. 为快捷指令命名，如「给他看」

                **使用方法**
                从控制中心或小组件运行该快捷指令。他下次调用「看一眼屏幕」时读取的即为该截图。

                ⚠️ **需手动运行。** iOS 不允许 App 在后台截取其他 App 的画面，无法由他主动获取。
                """))
                    .font(.app(11))
                    .foregroundStyle(Theme.textMuted(scheme))
                    .fixedSize(horizontal: false, vertical: true)
