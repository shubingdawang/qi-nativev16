...
struct ModelPickerView: View {
    ...
    经由电脑上的 Claude Code 调用，消耗订阅额度，不产生中转站费用。

    ⚠️ **需电脑保持开机**，新桥窗口关闭后无法连接。
    ⚠️ 额度**与工坊共用**，此处用量会占用工坊的可用额度。
    ⚠️ 工具调用经文字协议转发，稳定性低于中转站，偶有格式错误。
    ⚠️ 名称带 `-code` 的模型供工坊使用（可读文件、执行命令），不适用于聊天。
    """
...
struct SystemPromptView: View {
    ...
    /// 「立即压缩」那一下的确认框
    @State private var askCompact = false
        ...
        NavigationStack {
            Form {
                ...
                Section {
                    ...
                    // ⚠️ **这一下要问一句再做。**
                    //
                    // 她报的：「没有防误触。」——压缩会**多花一次请求**
                    // （她按次计价），而且压完之后那几十条原文就退成一份浓缩件，
                    // 退不回去。花钱又不可逆的事，不该点一下就发生。
                    Button {
                        askCompact = true
                    } label: {
                        HStack(spacing: 8) {
                            if compacting { ProgressView().scaleEffect(0.8) }
                            Text(compacting ? "正在压缩…" : "立即压缩")
                        }
                    }
                    .disabled(compacting)
                    .confirmationDialog("立即压缩？", isPresented: $askCompact,
                                        titleVisibility: .visible) {
                        Button("压缩") {
                            guard !compacting,
                                  let id = app.activeID(for: space) else { return }
                            compacting = true
                            Task {
                                ...
                                compactNote = ok
                                    ? "已生成浓缩件。"
                                    : "未生成浓缩件：条数未达下限（7 条），或模型未返回内容。"
                            }
                        }
                        Button("取消", role: .cancel) {}
                    } message: {
                        Text("本次压缩将额外产生一次请求。压缩后原文以浓缩件形式保留。")
                    }
                    HelpNote {
