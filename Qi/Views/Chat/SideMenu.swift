                            ...
                            row(item)
                                ...
                                .visualEffect { content, proxy in
                                    ...
                                    return content
                                        ...
                                        .offset(x: -d * d * 16)
                                        .opacity(1 - d * 0.68)
                                        // ⚠️⚠️ **这儿以前还有一句 `.blur(radius: d * d * 1.8)`。**
                                        //
                                        // 她报的：「左侧栏划出有点卡顿。」病根就是它。
                                        //
                                        // 缩放、位移、透明度都是仿射变换和 alpha，
                                        // GPU 上几乎不要钱；**模糊是真的要跑一遍卷积**，
                                        // 而这一层是**每一行各一份**——二十几行，
                                        // 而且滚动的每一帧都要按新的 `d` 重算。
                                        //
                                        // 它带来的观感只有「边上那几行虚一点」，
...
struct SideMenuItem: Identifiable, Hashable {
    ...
    static let all: [SideMenuItem] = [
        ...
        // 工资：排班日历 + 每天记账。跟备忘、承诺摆一起，都是她自己过日子的东西
        .init(id: "wage",      title: "工资", icon: "yensign.circle"),
