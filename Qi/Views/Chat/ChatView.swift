        ...
        AnyView(Group {
            TextField(app.settings.inputPlaceholder, text: draftBinding, axis: .vertical)
                ...
                .onChange(of: inputFocused) { _, on in
                    ...
                    //
                    // ⚠️⚠️ **这一下要等键盘先起来，不能当场做。**
                    //
                    // 她报的：「点开输入框有延迟。」病根就在这儿：
                    // 手指一落，`typingTick` 立刻变 → 整个消息列表重新求值
                    // （`typingTick` 在那道 `==` 里，一变就是全量重建）
                    // → 再跑一段带动画的滚动。**而 iOS 正在同一帧里弹键盘。**
                    // 两件重活撞在一起抢主线程，表现就是「按下去要等一下才亮」。
                    //
                    // 挪到 0.35 秒之后：键盘那段动画已经走完，
                    // 列表重建落在空档里，她感觉不到。
                    //
                    // ⚠️ 不能改成「不滚」——她明确要过这个滚动。
                    // 也不能只去掉动画：重建本身才是重的那一半。
        ...
        HStack(spacing: 0) {
            ...
            action("分享", Icon.share) {
                ...
                if let img = ShareCardRenderer.render(messages: picked, settings: app.settings) {
                    // 分享面板由 `DocPicker` 那个单例端出来。
                    // 挂在这一页上的话，`AppState` 一有动静就被撤掉。
                    DocPicker.shared.share([img])
                }
                selecting = false; selected = []
            }
