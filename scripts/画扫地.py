...
图纸 **40 格宽、27 格高**（顶上四行是留给举手的空，见 `加高图纸.py`）：
...
# ⚠️ 左右各补四列空：图纸从 32 格加宽到 40 格（见 `加宽图纸.py`）。
# 上面那些行还是照 32 格写的——**在这儿统一补**，
# 别去手改每一行，改漏一行整只就歪了。
...
FRAMES = [
    ('sweep1', frame(-1, 34, 24), 0.22, '甩到左边，身子跟着往左倾'),
    ('sweep2', frame(0, 34, 29), 0.16, '扫到中间，身子直起来'),
    ('sweep3', frame(1, 35, 33), 0.22, '甩到右边，身子往右倾'),
    ('sweep4', frame(0, 34, 29), 0.16, '收回中间。跟第二帧同一个姿势，'
