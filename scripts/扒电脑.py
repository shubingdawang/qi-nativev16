...
def main():
    ...
    for name, tap in (('working', 0), ('working2', 1)):
        ...
        # 手从肩膀垂下去，**贴着电脑两侧一直插到电脑中段**。
        #
        # ⚠️ 这是量他们的帧量出来的，不是估的。
        # 他们 SVG 里手臂绕肩膀转 70°~110°，转完落在 y 10..12，
        # 而笔记本占 y 9.5..15.5——也就是说**手臂跟电脑在竖直方向是重叠的**，
        # 手埋在电脑侧面，一直到电脑高度的四成左右。
        #
        # 我上一版让手垂到电脑上沿就停，等于两只手搭在电脑顶上悬着——
        # 她说的「手都没碰到电脑在那稀里糊涂的动」就是这个。
        hand_bottom = y0 + int(rows * 0.4) + tap
        for ax in (4, 32):
            for y in range(8, min(hand_bottom, len(g))):
