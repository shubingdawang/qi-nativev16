...
def bag(s):
    """小背包：**单肩背**。带子压在右臂上方，包挂在同一侧。

    ⚠️⚠️ 她纠正过两回，第二回说到根上了：

    > 现在左边的包带并没有和包联系上，而且你现在连接上的是 clawd 的头。
    > 包应该背在肩膀上，也就是手臂的上面，而不是头。

    对。这只螃蟹没有脖子，**躯干顶边 y=6 是「头顶」**——帽子才落那儿。
    「肩膀」是**手臂那条线**：手在 x0..2 / x13..15、y9..11，
    所以肩在 y≈8..9 的两侧。前几版的带子全挂在 y6，等于背在头上。

    她也给了退路：「如果你觉得斜挎的不好画，可以画成单肩背的，
    只用一边肩膀就可以背。」——就走这条。斜挎要横过整个身子，
    在这个尺寸上必然压到脸；单肩背整件都在右侧，一眼就读得出。
    """
    lit, mid, dim = "#C9A96A", "#8A6A3F", "#5E4628"
    # ⚠️⚠️ **带子压在右臂正上方**，照她的字面来：手臂在 x13..15、y9..11，
    # 「手臂的上面」就是 x13..15、y8..9 那一条。
    #
    # 上一版从 x11.3 斜下来两个单位，短得像个提手——
    # 那是因为我还想让带子看起来「从身上过来」，
    # 可在这个尺寸上，身上那一截既压眼睛又读不出是带子。
    # 干脆只画肩上那一段：**看得见的本来也只有搭在肩上的那一截。**
    s.rect(12.8, 8.05, 2.5, 0.95, dim)         # 搭在肩上那一段
    s.rect(12.8, 8.05, 2.5, 0.22, mid)         # 带面上一道亮边
    s.rect(12.8, 8.05, 0.35, 0.95, "#4A3620")  # 靠身子那头压出的暗边
    # 包身：挂在手臂**外侧**，吊在带子底下
    s.rect(13.4, 9.0, 2.9, 2.9, mid)
    s.rect(13.4, 9.0, 0.4, 2.9, lit)           # 左沿高光
    s.rect(13.4, 9.0, 2.9, 0.9, dim)           # 翻盖
    s.rect(13.4, 9.8, 2.9, 0.22, "#4A3620")
    s.rect(14.5, 9.55, 0.75, 0.48, lit)        # 扣子
    s.rect(13.4, 11.6, 2.9, 0.3, dim)          # 底边
    # 带子和包的接缝：**画在包之后**，把两块压成一件
    s.rect(13.8, 8.75, 1.5, 0.55, "#4A3620")
    s.rect(13.8, 8.75, 1.5, 0.15, mid)
