...
def bag(s):
    ...
    **整段都在两眼之间**，一只眼都不碰。
    ...
    # 斜挎带：左肩 → 右胯。
    # ⚠️ 步子(0.34)**比块(0.95×0.62)小**，块与块互相压住，斜边才不是锯齿。
    x0, y0 = 3.2, 6.0
    x1, y1 = 11.0, 11.2
    n = 26
    for i in range(n + 1):
        ...
        x = x0 + (x1 - x0) * t
        y = y0 + (y1 - y0) * t
        s.rect(x, y, 0.95, 0.62, dim)
        s.rect(x, y, 0.95, 0.16, mid)      # 带面朝上那一道亮边
    ...
    # 带子扎进包口那个扣：**画在包之后**，把两块压成一件
    s.rect(10.7, 10.45, 1.4, 0.6, "#4A3620")
    s.rect(10.7, 10.45, 1.4, 0.16, mid)
